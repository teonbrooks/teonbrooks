#!/usr/bin/env node
/**
 * Fixes the Meta position description's bullets to use bare present tense
 * (Lead/Standardize/Develop) instead of third-person "-s" forms
 * (Leads/Standardizes/Develops), matching the convention already used by
 * Gotham Data Clinic's ongoing bullets.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch24.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch24.mjs
 */
...
const NEW_DESCRIPTION = [
	'Lead the development of NeuralFetch and contribute to the NeuroAI open-source software suite to download, validate, and analyze multimodal neuroscience datasets (fMRI, MEG, EEG, iEEG, fNIRS, EMG).',
	'Standardize public datasets for ingestion and validate research studies using machine learning decoding techniques.',
	'Develop an automated validation and replication framework for neuroscience research projects.',
	'',
	'(Meta via Magnit)'
].join('\n');
...
async function main() {
	...
	const meta = await getRecord('id.sifa.profile.position', '3mizjdque242b');
	meta.description = NEW_DESCRIPTION;
	await putRecord(token, 'id.sifa.profile.position', '3mizjdque242b', meta, 'Meta — bullets to bare present tense');

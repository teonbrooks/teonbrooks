#!/usr/bin/env node
/**
 * Prefixes each consulting sub-role line in the datalus description with
 * "### " so they render as proper sub-headings (like Mozilla's merged
 * stints already do) with their sentences as subordinate bullets, instead
 * of the org/role/date line itself showing up as its own bullet.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch19.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch19.mjs
 */
...
async function main() {
	...
	await putRecord(token, 'id.sifa.profile.position', '3mvlmkhlfnx2o', datalus, 'datalus — add ### sub-headings for each consulting role');

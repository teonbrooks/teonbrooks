#!/usr/bin/env node
/**
 * Adds a blank line before "Advisor: ..." in the UNC Research Assistant
 * position description, matching the spacing convention used by every other
 * position with an Advisor line.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch15.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch15.mjs
 */
...
async function main() {
	...
	const unc = await getRecord('id.sifa.profile.position', '3mvlscl3adt2o');
	unc.description = unc.description.replace(' Advisor: Dr. Mohanish Deshmukh.', '\n\nAdvisor: Dr. Mohanish Deshmukh.');
	await putRecord(token, 'id.sifa.profile.position', '3mvlscl3adt2o', unc, 'UNC Research Assistant — add blank line before Advisor');

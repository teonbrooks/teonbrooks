#!/usr/bin/env node
/**
 * Removes the "Summary of the Event" header/separator line from the March
 * for Science involvement description, keeping the Wikipedia link.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch14.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch14.mjs
 */
...
async function main() {
	...
	const marchForScience = await getRecord('id.sifa.profile.involvement', '3mvdrhcezqe22');
	marchForScience.description = marchForScience.description.replace(
		'\n\nSummary of the Event\n------------------------------\n',
		'\n\n'
	);
	await putRecord(token, 'id.sifa.profile.involvement', '3mvdrhcezqe22', marchForScience, 'March for Science — remove "Summary of the Event" header');

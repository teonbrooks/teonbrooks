#!/usr/bin/env node
/**
 * Removes the "Cognitive Science: ..." description line from the Master's
 * Degree education record per Teon's request.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch13.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch13.mjs
 */
...
async function main() {
	...
	const masters = await getRecord('id.sifa.profile.education', '3mizjdquezr2b');
	delete masters.description;
	await putRecord(token, 'id.sifa.profile.education', '3mizjdquezr2b', masters, "Master's Degree — remove description");

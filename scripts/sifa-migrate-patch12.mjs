#!/usr/bin/env node
/**
 * Renames the datalus position's company field from "datalus, LLC" to
 * "datalus" per Teon's request.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch12.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch12.mjs
 */
...
async function main() {
	...
	datalus.company = 'datalus';
	await putRecord(token, 'id.sifa.profile.position', '3mvlmkhlfnx2o', datalus, 'datalus, LLC — rename company to "datalus"');

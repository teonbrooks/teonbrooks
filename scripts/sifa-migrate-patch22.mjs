#!/usr/bin/env node
/**
 * Fixes the datalus description's OpenBCI sub-heading to use a year-only
 * date range, matching the BrainWaves and MSK sub-headings.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch22.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch22.mjs
 */
...
async function main() {
	...
	await putRecord(token, 'id.sifa.profile.position', '3mvlmkhlfnx2o', datalus, 'datalus — use year-only date range for OpenBCI sub-heading');

#!/usr/bin/env node
/**
 * Adds the division ("FAIR") to the Meta position title. The "(Contract)"
 * suffix is deliberately NOT added here — employmentType is already
 * correctly set to "contract" on this record, and the export script now
 * derives the display suffix from that field automatically.
 *
 * Usage:
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch16.mjs --dry-run
 *   SIFA_IDENTIFIER=teonbrooks.com SIFA_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/sifa-migrate-patch16.mjs
 */
...
async function main() {
	...
	meta.title = 'Data Scientist, FAIR';
	await putRecord(token, 'id.sifa.profile.position', '3mizjdque242b', meta, 'Meta — add FAIR division to title');
